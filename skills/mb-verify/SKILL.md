...
- For `T2`, `mb-verify` records functional evidence and can make task closure eligible when full protocol and applicable task/spec gates are satisfied; product feature completion still requires feature-level `mb-red-verify --feature FT-<ID>` semantic-pass recorded in the feature doc. `FT-000` foundation closure uses normal task dependencies and the final foundation gate. For `T3`, final task closure requires per-task `mb-red-verify` semantic-pass first.
...
- verify task gates/checks/evidence requirements are covered
...
- apply status by tier:
  ...
  - manual mode: may set `T0` / `T1` `status: done` after functional `VERDICT: PASS` with explicit closure ownership; for `T2`, recommend task closure when full protocol and applicable task/spec gates are satisfied; for `T3`, leave closure pending per-task `/red-verify` `SEMANTIC_VERDICT: semantic-pass`
...
- PASS verification has updated RTM/task evidence; `T2` tasks can close without per-task red-verify when full protocol, applicable task/spec gates, and PASS evidence are present; `T2` features are not complete until feature-level red-verify produces semantic-pass recorded in the feature doc; `T3` tasks are not closed until per-task `/red-verify` / `mb-red-verify` produces `semantic-pass`.
