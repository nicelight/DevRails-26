...
protocol/evidence for verification and the lifecycle owner. `/execute` is not a
scheduler.
</objective>
...
- Scheduler mode: `/execute` never writes final task closure/failure/blocking,
...
- T2/T3 task closure is never owned by `/execute`.
...
- Feature/task repair or tier rebuild routes to
  `/prd-to-tasks FT-<NNN>`; shared/global design routes to `/spec-design`;
  product clarification routes to `/clarify-feature FT-<NNN>`.
...
- T2/T3: `.protocols/<TASK_ID>/{context,plan,progress,verification,handoff}.md`
  using available templates; `/execute` fills implementation context/progress
...
`/prd-to-tasks FT-<NNN>` for controlled rebuild/split, followed by
`/review-tasks-plan`, applicable doctor gate, and `/execute <replacement-id>`.
</validation>
