...
Manual execution priming for `/execute-task TASK-NNN-TN-FT-NNN-WN`:
1. Read `AGENTS.md` (this guide)
...
- If `/execute-task` or `/verify` discovers a required higher tier, stop scope growth and route
  the original task ID through `/feature-to-tasks FT-<NNN>` for controlled
...
- If running in **Codex**: you can run each `TASK-NNN-TN-FT-NNN-WN` in a fresh session via `codex exec` (see `/execute-task`).
...
- `/execute-task` may use linked behavior specs as context examples; `/verify` does
