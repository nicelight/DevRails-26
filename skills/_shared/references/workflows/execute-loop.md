...
1) `/grill-me -> /constitution` when raw or uncertain product discovery is needed; a clear authoritative product source may start at `/constitution` or `/write-prd` according to Constitution state
