...
name: mb-red-verify
description: >
  Adversarial semantic verification for one TASK-NNN-TN-FT-NNN-WN so teams can
  catch solutions that pass process checks but are still wrong in substance.
---

# mb-red-verify — Adversarial semantic verification

- **What it does:** pressure-tests one completed `TASK-NNN-TN-FT-NNN-WN` for semantic correctness, hidden failure modes, and systemic harm.
