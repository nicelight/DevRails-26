...
2. при разложении одной feature на JSON tasks — до выпуска task cards проверять
   ...
   работу, из-за которой задача раздуется во время `/exe`.
...
время `/exe`, tier escalation или controlled `rebuild_required`.
...
- `/exe` реже обнаруживает oversized scope, missing prerequisite или
...
- число `/exe` stop из-за отсутствующей prerequisite work или plan-level
  decisions;
