...
2. при разложении одной feature на JSON tasks — до выпуска task cards проверять
   ...
   работу, из-за которой задача раздуется во время `/execute-task`.
...
- `/prd-to-features` смотрит на продукт целиком и одновременно должен обеспечить полное
...
- `/review-feat-plan` использует fresh context, но проверяет всю L1-L3
  ...
  dimension;
- `/feature-to-tasks` объединяет feature-level SDD coverage, implementation plan и
  выпуск полной JSON task queue;
- single-feature режим `/feature-to-tasks FT-<NNN>` уже существует, но unattended
  flow использует `/spec-auto --all` и `/feature-to-tasks --all`, снова расширяя
  рабочий контекст;
...
время `/execute-task`, tier escalation или controlled `rebuild_required`.
...
`/prd-to-features` остаётся владельцем product decomposition и сначала строит полную карту.
...
index. Focused analysis остаётся внутренней тактикой существующего `/prd-to-features`, без
...
`/feature-to-tasks`, после preliminary slicing и до выпуска JSON task records.
...
`/feature-to-tasks FT-<NNN>` или другому owning skill.
...
- полный model-eval framework как обязательный verification gate;
...
- `/prd-to-features` остаётся владельцем product/REQ/epic/feature decomposition.
- `/spec-design` остаётся mandatory global SDD backbone и Foundation decision
  gate.
- `/feature-to-tasks` остаётся planning skill и не выполняет tasks.
...
- agent-run verification — только минимальные deterministic contract assertions
  и изолированные source-only/install smoke checks.
...
Focused analysis остаётся внутренней тактикой существующих `/prd-to-features`,
`/review-feat-plan`, `/feature-to-tasks` и `/review-tasks-plan`; новые modes и
...
- `/execute-task` реже обнаруживает oversized scope, missing prerequisite или
...
- число `/execute-task` stop из-за отсутствующей prerequisite work или plan-level
  decisions;
