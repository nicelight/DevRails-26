...
- читает `.memory-bank/roles/worker.md` и сохраняет роль;
...
1. Прочитать локальный `AGENTS.md`, worker role и skill целевого workflow.
...
2. Заменить `Explorer TODO` в canonical worker role компактным Explorer
   contract.
...
- worker/orchestrator contracts задают один недублированный handoff;
