...
Из-за дублирования `mb-lint` может остановить doctor до применения его
mode-specific policy, а два набора regex/checks могут разойтись.
...
2. Валидная T0/T1 `done` task без closure evidence проходит `mb-lint`, но
   получает соответствующий `mb-doctor` finding.
3. Валидная T2/T3 `done|failed` task без full protocol или PASS/FAIL evidence
   проходит `mb-lint`, но блокируется `mb-doctor --strict`.
4. Полностью согласованная terminal task проходит lint и применимый doctor
   mode.
5. Scheduler closure rules и verify/red-verify verdict ownership не меняются.
