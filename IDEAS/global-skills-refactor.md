...
- Основной scope: canonical command skills, shared workflow references,
  generated runtime skill behavior и agent-run verification
...
- обязательное положение после `/prd-to-features`;
...
- повторяет значительную часть `/spec-design` и `/feature-to-tasks`;
...
verification, `/execute-task` останавливается и спрашивает оператора либо возвращает
...
2. Отметить поля/строки, которые механически читает `mb-lint`, `mb-doctor` или
   installer и которые должны покрываться agent-run checks.
...
- `/execute-task`, `/verify`, `/red-verify`, scheduler и `/mb-sync` сохраняют текущую
  ownership model.
...
6. `/feature-to-tasks` свободно выбирает analysis order, но создаёт schema-valid
   complete task cards.
...
- generated DevRails target содержит согласованные runtime skills и workflow
  references;
- agent-run syntax/install/bootstrap/lint checks и behavioral fixtures проходят;
...
5. Провести `/review-tasks-plan`-подобный fresh-context review самого refactor
   результата перед переходом к `/spec-design` и `/feature-to-tasks` slimming.

После подтверждения этого slice тем же pattern последовательно упростить
`/spec-design`, `/feature-to-tasks`, `/foundation-to-tasks`, `/execute-task`, review и
