...
- минимальные RF-01A assertions в existing release check.
...
- selective `cold-start`, fresh bootstrap, explicit sync и existing-AGENTS
  fixtures в existing release check.
...
- release check получает только минимальные ordering/ownership assertions.
...
- targeted assertions/fixtures в existing release check.
...
- behavior change:
  `skills/_shared/references/commands/autonomous.md`;
- minimal exact-number/off-by-one assertions в existing release check;
...
- RF-12 требует менять что-либо кроме canonical `/autonomous` и минимальных
  assertions в existing release check;
