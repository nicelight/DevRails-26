...
Статус документа: review notes; finding №1 реализован и проверен, остальные
пункты остаются без реализации.
...
При исходном повторном аудите installer/bootstrap не запускались. Для finding
№1 после одобрения решения выполнены isolated install/bootstrap/sync smoke и
regression test; для остальных пунктов `подтверждён` означает только source code
и contract evidence, а не runtime smoke result.
...
| 2 | подтверждён source chain | 5/5 | 2/5 | 1/5 | 3/5 | P0 |
...
| 7 | подтверждён прямым конфликтом template/policy | 4/5 | 1/5 | 1/5 | 2/5 | P1 |
...
**Вердикт:** исправлен узким P0 slice без общей mixed-file migration:
синхронизируются только доказанные framework-owned contracts, а полный
installer upgrade route согласован по commands и Memory Bank assets.
...
Дополнительный route defect: canonical `/mb-init` предлагает explicit sync
через `--bootstrap-only --sync`. После обновления schema такой route может
создать обратный skew — новые Memory Bank contracts при старых runtime command
skills. Полный framework upgrade должен обновлять commands и Memory Bank assets
из одной prepared source copy через `--bootstrap --sync`.
...
- `.memory-bank/mbb/index.md` и остальные inline skeleton docs после bootstrap
  считаются seed-once/project-customizable. Version-critical framework rules
  нельзя размещать только в них: они должны находиться в обновляемых canonical
  command/workflow/role references;
- generated blocks для factual installed-skill registry принадлежат finding
  №9. До него `/mb-garden` остаётся существующим mechanical reconciliation
  route для project routers.

### Оценка

- важность: `5/5` — existing installations могут получить внутренне
  несовместимые commands, schema и framework policies;
- сложность: `2/5` для одобренного P0 slice; mixed-router migration оценивается
  отдельно в finding №9;
- риск раздувания: `2/5` при сохранении узкой ownership allowlist;
- объём: `3/5` — generator, installer route/docs и isolated regression smoke.

### KISS-исправление

1. В `init-mb.js` явно записывать task schema как whole-file framework-owned с
   `overwrite: SYNC_MODE`. Не вводить registry ради одного call site. Canonical
   copied workflows/roles/scripts уже имеют overwrite path; protocol templates
   должны использовать тот же pattern после finding №2.
2. Project-owned и mixed files, включая routers, task records и existing
   testing policy, оставлять без изменений. Generated router blocks не входят в
   этот slice.
3. `writeFile` должен различать `created|updated|unchanged|kept`, а sync report —
   честно группировать framework-owned updates и сохранённые project/mixed
   files. Одинаковое содержимое не считать update.
4. Explicit framework sync в canonical `/mb-init` направлять через
   `install-framework.mjs --bootstrap --sync`, чтобы commands и Memory Bank
   contracts происходили из одной prepared copy.
5. `--bootstrap-only --sync` оставить только как явно обозначенный repair route
   для Memory Bank managed assets; он не является полным framework upgrade и не
   обновляет runtime command skills.

Не вводить runtime status/gate, общий three-way merge, versioned migration
registry или скрытое определение ownership по произвольному содержимому.
Installer sync не является runtime `/mb-sync` и не должен вызывать или менять
его workflow contract.

### Поверхность изменений

- `skills/_shared/scripts/init-mb.js`;
- `scripts/install-framework.mjs` для ясной bootstrap-only repair semantics;
- `skills/{mb-init/SKILL.md,_shared/references/commands/mb-init.md}`;
- `README.md` и `howItWorks.md` для точного sync ownership contract;
- regression smoke: stale schema, command/schema coherence, unchanged second
  sync, custom router entries и project-owned docs.

### Acceptance

- stale schema обновляется полным `--bootstrap --sync`;
- runtime commands, schema и canonical copied assets происходят из одной
  prepared source copy;
- повторный sync классифицирует identical framework assets как `unchanged`;
- custom router entries и project-authored state сохраняются;
- intentional testing-policy exception сохраняется;
- `--bootstrap-only --sync` явно сообщает, что commands не обновляются;
- report соответствует фактическому действию по каждому классу ownership.

Non-backward-compatible изменение task schema не должно молча переписывать
project task records. Для конкретной released legacy shape требуется отдельная
bounded migration или blocker до release; общий migration framework не нужен.

### Реализация и проверка

- task schema обновляется как explicit whole-file framework-owned asset;
- sync report различает `created|updated|unchanged|kept` по ownership;
- canonical `/mb-init` использует `--bootstrap --sync` для полного upgrade;
- `--bootstrap-only --sync` сообщает repair-only semantics;
- regression smoke покрывает stale schema, command/schema coherence,
  idempotent sync и сохранение custom routers/project state;
- isolated install-only, fresh bootstrap и `mb-lint` проходят; source-only
  `shared-*` count остаётся `0`.
...
1. **Sync/deployment ownership:** finding 1 закрыт; затем findings 2 и 9 —
   protocol templates и factual registry block на уже определённой ownership
   policy.
...
3. **Manual lifecycle:** findings 5 и 7. Сначала start ownership/attempt ordering,
