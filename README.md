...
> `DevRails 26` — **это фреймворк в связке с Codex и Claude, разворачиваемый внутри проекта в формате долгосрочной памяти и набора взаимосвязанных протоколов для полуавтоматической разработки.**
...
После развёртывания DevRails в папке проекта — частично разработанного или находящегося на стадии идеи — разработка превращается в мягко направляемое агентами, но полностью контролируемое Вами движение по всем необходимым этапам: от мозгового штурма до e2e-тестов.

Плавную работу обеспечивают взаимосвязанные runtime-skills, JSON task registry и Memory Bank, работающие поверх бесшовных handoff-протоколов.
...
- Можно запустить автопилот, предварительно пройдя brainstorming flow только для формирования PRD. В автопилоте всё же придётся уделять внимание Оркестратору раз в пару часов, принимая решения на развилках. Такой процесс жрёт куда больше токенов, результат может получиться местами с overhead-кодом, но будет доведён до завершения, максимально экономя время разработчика.
...
- `.agents/skills/` и `.claude/skills/` — 33 команды DevRails для Codex и
Claude Code;
