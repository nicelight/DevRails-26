...
/brief
  ...
  -> /execute + /verify до закрытия Foundation Gate
  -> /prd-to-tasks FT-<NNN>
...
inferences. Без PRD или delta он не создаёт roadmap epics, product features или
runnable task records.

## Task execution по tiers

| Tier | Тип работы | Manual route | Обязательный результат |
|---|---|---|---|
| `T0` | safe docs-only | `/execute` | compact evidence; closure только у explicit top-level owner |
| `T1` | local code или contained behavior | `/execute`, optional `/verify` | relevant local check или объяснение, почему runnable check не нужен |
| `T2` | API, contract, state/data/domain, cross-module | `/execute -> /verify` | full protocol + functional PASS; feature-level `/red-verify --feature FT-*` перед completion |
| `T3` | security, production, irreversible/data-loss, payments, compliance | `/execute -> /verify -> /red-verify` | functional PASS + semantic-pass + exact `HUMAN_CHECKPOINT: done` |

`touched_files` — advisory, non-exhaustive прогноз. Фактический write set
подтверждает `/execute` preflight. Только непустой
...
task возвращают в `/prd-to-tasks FT-<NNN>` для controlled rebuild/split, затем
...
terminal state. `/execute`, `/verify` и `/red-verify` возвращают evidence и
...
- `/cold-start` — определяет greenfield, brownfield или skeleton-only route.
- `/mb-init` — создаёт или синхронизирует skeleton Memory Bank.
- `/mb` — загружает минимально достаточный project context.
...
- `/clarify-feature` — опционально снимает ambiguity одной feature.

### Product, SDD и task planning

- `/spec-init` — готовит pre-PRD domain/scenario/boundary framing.
- `/prd` — декомпозирует clarified PRD в product, `REQ-*`, epics и `FT-*`.
...
- `/foundation-to-tasks` — создаёт minimum `FT-000` queue или доказывает
  existing baseline.
- `/prd-to-tasks` — согласует feature-level canonical specs, implementation
...
- `/execute` — реализует одну indexed task в её semantic и hard boundaries.
...
- `/mb-garden` — устраняет Memory Bank drift и поддерживает индексы/ссылки.
