...
const WEB_DESIGN_REVIEWER_PACK = {
  sourceRel: join(
    'skills',
    '_shared',
    'references',
  ...
  targetRel: join('references', 'web-design-reviewer.md'),
};
const FINDING_ADJUDICATION_PACK = {
  sourceRel: join(
    'skills',
    '_shared',
    'references',
  ...
  targetRel: join('references', 'finding-adjudication.md'),
};
const CODE_REVIEWER_PROMPT = {
  sourceRel: join('skills', '_shared', 'agents', 'review-code.md'),
  targetRel: join('agents', 'review-code.md'),
};
const COMMAND_RESOURCE_SPECS = new Map([
  ['red-verify', [WEB_DESIGN_REVIEWER_PACK, FINDING_ADJUDICATION_PACK]],
  ['review-feat-plan', [FINDING_ADJUDICATION_PACK]],
  ['review-tasks-plan', [FINDING_ADJUDICATION_PACK]],
  ['verify', [
    WEB_DESIGN_REVIEWER_PACK,
    FINDING_ADJUDICATION_PACK,
  ...
  return listMarkdownFiles(commandDir)
    .map((filename) => {
      ...
      return {
        ...
        resources: (COMMAND_RESOURCE_SPECS.get(name) || []).map(({ sourceRel, targetRel }) => {
          const sourcePath = join(preparedRepo, sourceRel);
          if (!existsSync(sourcePath)) {
            console.error(`Missing runtime skill resource: ${sourcePath}`);
            process.exit(1);
          }
          return {
            targetRel,
            content: readFileSync(sourcePath, 'utf8').replace(/\r\n/g, '\n'),
          };
        }),
      };
    })
...
function writeRuntimeSkill(targetRepo, runtimeRoot, spec) {
  ...
  spec.resources.forEach((resource) => {
    const resourcePath = join(skillDir, resource.targetRel);
...
async function runInstallFlow({ targetRepo, install, bootstrap, syncMode, interactive = false, agentsPolicy = 'keep' }) {
  ...
  try {
    ...
    if (interactive) {
      ...
      if (bootstrap) {
        uiCardLine();
        uiCardLine(`${uiTone('Следующий шаг', 'muted')}  ${uiKeycap('/start')}`);
      }
      uiCardEnd();
    } else {
      console.log('\nУстановка завершена.');
      ...
      if (bootstrap) console.log('  Следующий шаг: /start');
    }
  } finally {
