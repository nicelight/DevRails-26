...
try {
  ...
  assert(
    readFileSync(join(agentsCollisionDir, 'SKILL.md'), 'utf8') === customAgentsSkill
      ...
      && !existsSync(join(collisionTarget, '.agents', 'skills', 'start'))
      && !existsSync(join(collisionTarget, '.claude', 'skills', 'start')),
  ...
  ['.agents', '.claude'].forEach((runtimeRoot) => {
    ...
    const renamedSkillDir = join(legacyProxyTarget, runtimeRoot, 'skills', 'cold-start');
    mkdirSync(renamedSkillDir, { recursive: true });
    writeFileSync(
      join(renamedSkillDir, 'SKILL.md'),
  ...
  ['.agents', '.claude'].forEach((runtimeRoot) => {
    ...
    assert(
      installOnlyStart.includes(fullBootstrapRoute)
        && !installOnlyStart.includes(skeletonBootstrapRoute),
      `${runtimeRoot} install-only start does not route missing skeleton to full bootstrap.`,
    );
  });
  ...
  assert(
    freshAgents.includes('/start'),
    'Generated AGENTS.md lost its default runtime entry.',
  );
  assert(
    expectedRuntimeSkill.includes('<routing_contract>')
      && expectedRuntimeSkill.includes('execute-loop.md#interactive-mode-you-stay')
      && expectedRuntimeSkill.includes('<validation>'),
    'Deployed start is not a single-owner state router.',
  );
  assert(
    autopilotSkill.includes('autonomy-policy.md#scheduler-failure-handling')
      && autonomousSkill.includes('`#scheduler-failure-handling`'),
    'Scheduler runtime skills do not route failure/retry semantics to autonomy policy.',
  );
  assert(
    exeSkill.includes('## Execution Attempt')
  ...
  rmSync(join(driftTarget, '.claude/skills/start/SKILL.md'), { force: true });
  ...
  assert(
    readFileSync(driftSkillIndexPath, 'utf8').includes('| <code>start</code> | yes | no |'),
  ...
  assert(
    migratedSkillIndex.includes('| <code>start</code> | yes | yes |')
