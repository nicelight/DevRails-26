...
| `/prd-to-tasks` | Initial product feature concern design or safe reconciliation -> implementation plan + complete JSON tasks | registry-first concern audit; reuse/extend/create-only-missing subject-based canonical specs; direct relevant spec links in feature/task cards; optional `.memory-bank/behavior-specs/*.behavior.json`; `.memory-bank/tasks/plans/IMPL-FT-*.md`; indexed product `TASK-NNN-TN-FT-NNN-WN` records with T2/T3 single-card handoff | не создаёт default `FT-*` spec hubs; не принимает новые shared/global design decisions; не запускает execution; при reconciliation не меняет task identity/lifecycle/evidence; не проходит competing canonical paths, pending blockers, missing concrete blocks, `FT-000`, or missing required foundation gate | `/review-tasks-plan FT-*`, conditional `/mb-doctor` for complex/T3/autonomous boundaries, then tier-routed `/execute`; or `/autopilot` |
